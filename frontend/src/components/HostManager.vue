<template>
  <div class="host-manager p-4 bg-gray-800 text-white rounded-lg shadow-lg">
    ...
    <!-- Host Form -->
    <form @submit.prevent="saveHost" class="mb-8 p-6 bg-gray-700 rounded-md shadow-inner">
      <h3 class="text-xl font-semibold mb-4">{{ editingHost ? 'Edit Host' : 'Add New Host' }}</h3>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label for="alias" class="block text-sm font-medium text-gray-300">Alias:</label>
          <input type="text" id="alias" v-model="form.alias" required
        ...
        <div>
          <label for="hostname" class="block text-sm font-medium text-gray-300">Hostname/IP:</label>
        ...
        <div class="md:col-span-2">
          <label for="password" class="block text-sm font-medium text-gray-300">Password (optional):</label>
      ...
      <div class="flex justify-end space-x-2 mt-6">
        ...
        <button v-if="editingHost" @click="cancelEdit" type="button"
    ...
    <!-- Error Display -->
    <div v-if="hostStore.error" class="bg-red-900 text-white p-3 rounded-md mb-4">
    ...
    <div class="host-list">
      ...
      <div v-if="hostStore.isLoading" class="text-center text-gray-400">Loading hosts...</div>
      <ul v-else class="space-y-3">
        <li v-for="host in hostStore.hosts" :key="host.id"
        ...
        <li v-if="hostStore.hosts.length === 0 && !hostStore.isLoading" class="text-center text-gray-400 p-4">
